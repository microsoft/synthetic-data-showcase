use super::stats::{
    calc_fabricated_combinations_percentage, calc_mean_combinations_count_error,
    calc_mean_combinations_count_error_by_len, calc_mean_combinations_length_by_bucket,
    calc_mean_proportional_error, calc_mean_proportional_error_by_bucket,
    calc_original_mean_combinations_count, calc_original_mean_combinations_count_by_len,
    calc_suppressed_combinations_percentage,
};
use crate::{
    processing::aggregator::aggregate_result::WasmAggregateResult,
    utils::js::ts_definitions::{JsAggregateCountsStatistics, JsResult},
};
use sds_core::processing::{aggregator::typedefs::AggregatedMetricByLenMap, evaluator::Evaluator};
use serde::Serialize;
...
pub struct WasmAggregateCountsStatistics {
...
#[wasm_bindgen]
impl WasmAggregateCountsStatistics {
    ...
    pub fn from_aggregate_results(
        ...
        resolution: usize,
    ) -> JsResult<WasmAggregateCountsStatistics> {
        ...
        Ok(WasmAggregateCountsStatistics {
    ...
    pub fn to_js(&self) -> JsResult<JsAggregateCountsStatistics> {
        Ok(JsValue::from_serde(self)
            .map_err(|err| JsValue::from(err.to_string()))?
            .unchecked_into::<JsAggregateCountsStatistics>())
    }
}
